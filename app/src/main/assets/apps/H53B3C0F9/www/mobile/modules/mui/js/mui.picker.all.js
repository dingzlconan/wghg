...
(function($, window, document, undefined) {
	...
	Picker.prototype.triggerChange = function(force) {
		...
		setTimeout(function() {
			...
			// add by dingzl 2018/5/30
			if ('undefined'==typeof(force)){
				if (self.options.ontriggerChange)
					self.options.ontriggerChange(self,function(data){
						var combo = self.options.target;
						if (combo.options.async) {
							var items = combo.picker.getSelectedItems();
							var va = [];
							for (var i=0;i<self.options.sequence;i++) {
								va.push(items[i].value);
							}
							var odata = combo.getData();
							var ndata = (function appenddata(odata,va,data,i){
								return odata.map(function(v,k,a){
									if (v.value==va[i]) {
										if (va.length>i+1) {
											v.children=appenddata(v.children,va,data,i+1)
										} else {
											v.children = data;
										}
										return v;
							...
							var nndata = (function dwdata(data,idField,textField) {
								...
								return data;
							})(ndata,combo.options.idField,combo.options.textField);
							combo.picker.setData(nndata);
							if(combo.options.async&&combo.options.value){
								var value = combo.options.value;
								var va = value.split(",");
								setTimeout(function(){
									combo.picker.pickers[self.options.sequence].setSelectedValue(va[self.options.sequence])
										if (self.options.sequence+1==combo.options.layer) {
											setTimeout(function(){
												var id = combo.id;
												var items = combo.picker.getSelectedItems();
												var va = [];
												...
												if (!combo.options.asyncload) {
													document.getElementById(id).value=va.join(",");
												}
												combo.options.finished = true;
												if (combo.options.onLoadSuccess)
													combo.options.onLoadSuccess();
											},200);
										}
								},200);
							} else {
								combo.picker.pickers[self.options.sequence].setSelectedIndex(0,200,function(){
									if (self.options.sequence+1==combo.options.layer) {
										setTimeout(function(){
											var id = combo.id;
											combo.options.finished = true;
